...
from .services.sgnipt import sgnipt_host_path_candidates
...
def _sgnipt_log_candidates(job: Job) -> List[str]:
    ...
    for base in (
        ...
        if not base:
            continue
        for candidate in sgnipt_host_path_candidates(os.path.join(base, _LOG_NAME_NEXTFLOW)):
            _add_candidate(seen, out, candidate)
